...
export class SignupModal extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    Promise.all([
      ...
      fetch('https://api.mlab.com/api/1/databases/tdm/collections/user?q={"city":"SF"}&apiKey=9zaMF9-feKwS1ZliH769u7LranDon3cC', {method: 'GET',}).then(res => res.json())
    ])

      .then(([loginStatus, countres]) => {
        if (countres[0].count % 2 === 0) {
          window.location.href='http://coaxs.herokuapp.com/main/sf/presurveyaccess?email='+this.state.email
        } else {
          window.location.href='http://coaxs.herokuapp.com/main/sf/presurveyptp?email='+this.state.email
        }
      });
